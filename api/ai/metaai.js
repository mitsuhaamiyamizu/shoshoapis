...
class MetaBot {
    ...
    async fetchCookies() {
        const response = await axios_1.default.get("https://www.meta.ai/", {
    ...
    async fetchToken(birthDate = "1990-01-01") {
        ...
        try {
            const response = await axios_1.default.post("https://www.meta.ai/api/graphql", new URLSearchParams(payload).toString(), { headers });
    ...
    async scrape(message) {
        ...
        try {
            const response = await axios_1.default.post("https://graph.meta.ai/graphql?locale=user", body.toString(), { headers });
    ...
    isValidResponse(botMessage) {
        return (botMessage.streaming_state === "OVERALL_DONE" ||
