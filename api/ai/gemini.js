...
class GeminiAPI {
    ...
    log(message) {
        if (this.config.debug)
            console.log(`\x1b[36m[GeminiAPI]\x1b[0m \x1b[35m${message}\x1b[0m`);
    }
    isUrl(str) {
        try {
            new url_1.URL(str);
    ...
    async downloadFile(url) {
        ...
        let fileName = path_1.default.basename(urlObj.pathname);
        if (!fileName || !path_1.default.extname(fileName)) {
            const contentType = response.headers['content-type'];
            fileName = `file_${Date.now()}${this.getExtFromMime(contentType)}`;
        }
        return {
            buffer: buffer_1.Buffer.from(response.data),
            fileName
        };
    }
    ...
    async fetchWizData() {
        ...
        try {
            const response = await axios_1.default.get(this.initialUrl, { headers: this.headers });
    ...
    async uploadImage(fileName, fileBuffer) {
        ...
        try {
            ...
            const startResponse = await axios_1.default.post(this.uploadUrl, `File name: ${fileName}`, { headers: uploadHeaders });
            ...
            const uploadResponse = await axios_1.default.post(uploadUrl, fileBuffer, { headers: uploadFileHeaders });
    ...
    async fetchImageUrl(rawImageUrl) {
        ...
        const imageHeaders = {
            authority: new url_1.URL(rawImageUrl).hostname,
        ...
        try {
            const firstResponse = await axios_1.default.get(rawImageUrl, {
            ...
            const secondHeaders = {
                authority: new url_1.URL(redirectUrl).hostname,
                accept: "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
                "accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
                cookie: this.config.cookie,
                "sec-ch-ua": "\"Not A(Brand\";v=\"8\", \"Chromium\";v=\"132\"",
            ...
            const secondResponse = await axios_1.default.get(redirectUrl, {
                headers: secondHeaders,
                maxRedirects: 0,
                validateStatus: (status) => status === 302,
            });
            const finalUrl = secondResponse.headers.location;
            if (!finalUrl) {
    ...
    async query(query, options = {}) {
        ...
        let response;
        try {
            response = await axios_1.default.post(this.streamUrl, new URLSearchParams(data).toString(), { headers: this.headers, params, responseType: 'stream' });
        }
        catch (error) {
        ...
        let messageText = "", newConversationID = null, newResponseID = null, newChoiceID = null, rawImageUrl = null, youtubeUrl = null, ImageSearch = null;
...
async function scrape(text, cookie, options = {}) {
    const { promptSystem = "", imageUrl = null, conversationID = null, responseID = null, choiceID = null, } = options;
    ...
    if (imageUrl) {
        try {
            ...
            file = buffer_1.Buffer.from(imageResponse.data);
        }
        catch (error) {
        ...
        async run({ req }) {
            ...
            try {
                ...
                if (result.conversationID)
                    headersToSend["conversationID"] = result.conversationID;
                if (result.responseID)
                    headersToSend["responseID"] = result.responseID;
                if (result.choiceID)
                    headersToSend["choiceID"] = result.choiceID;
                return {
                    status: true,
                    data: {
