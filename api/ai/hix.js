        ...
        try {
            ...
            const handlers = {
                chat: async () => {
                    ...
                    if (payload.keyword.length < 2)
                        return {
                            ...
                            code: 400,
                            type,
                            message: "Keywordnya kependekkan bree, minimal 2 karakter dong biar resultnya lebih gacor 😂",
                            payload: { current: payload, required: { keyword: "Minimal 2 karakter" } },
                        };
                    const response = await axios_1.default.post(`${hika.api.base}${type === "chat" ? hika.api.kbase : hika.api.advanced}`, payload, { headers });
                    if (!response.data)
                        return {
                            status: false,
                            code: 404,
                ...
                mindmap: async () => {
                    ...
                    const response = await axios_1.default.post(`${hika.api.base}${hika.api.mindmap}`, payload, { headers });
                    const result = hika.parse(response);
                ...
                keywords: async () => {
                    ...
                    const response = await axios_1.default.post(`${hika.api.base}${hika.api.keywords}`, payload, { headers });
                    const result = hika.parse(response);
                    return { status: true, code: 200, data: { type, text: result.text } };
                },
            };
...
async function scrapeHikaChat(keyword, language = "id") {
    try {
...
exports.default = [
    {
        ...
        async run({ req }) {
            ...
            try {
                const result = await scrapeHikaChat(keyword.trim(), language || "id");
                if (!result) {
                    return {
                        status: false,
