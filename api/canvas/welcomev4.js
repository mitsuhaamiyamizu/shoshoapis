...
async function generateWelcomeImageV4FromURL(avatar, background, description) {
...
async function generateWelcomeImageV4FromFile(avatarBuffer, backgroundBuffer, description) {
    const image = await new canvafy.WelcomeLeave()
...
exports.default = [
    {
        ...
        example: "?avatar=https://i.ibb.co/1s8T3sY/48f7ce63c7aa.jpg&background=https://i.ibb.co/4YBNyvP/images-76.jpg&description=Welcome%20friend!",
        parameters: [
            {
                name: "avatar",
                in: "query",
    ...
    },
    {
        ...
        async run({ req, guf }) {
            ...
            try {
                const image = await generateWelcomeImageV4FromFile(avatarFile.file, backgroundFile.file, description.trim());
                return createImageResponse(image);
            }
            catch (error) {
                return {
